...
from schemas.comentario import ListagemComentariosSchema, ComentarioBuscaSchema, apresenta_comentarios
...
home_tag = Tag(name="Documentação das APIs", description="Seleção de documentação: Swagger, Redoc ou RapiDoc")
produto_tag = Tag(name="Produto", description="Adição, visualização e remoção de produtos à base")
comentario_tag = Tag(name="Comentario", description="visualização, Adição de um comentário à um produtos cadastrado na base")
...
def get_produto(query: ProdutoBuscaSchema):
    ...
    Retorna uma representação dos produtos e comentários associados a partir de um id do produto.
    É o detalhe de cada produto. Esse comentário escrito na app.py aparece na documentação!!!
    """
...
@app.get('/comentarios', tags=[comentario_tag],
         responses={"200": ListagemComentariosSchema, "404": ErrorSchema})
def get_comentarios(query: ComentarioBuscaSchema):
    """Faz a busca por todos os comentários cadastrados
    ...
    session = Session()
    # fazendo a busca
    comentarios = session.query(Comentario).filter(Comentario.produto == produto_id).all()
    ...
    if not comentarios:
        ...
        # logger.debug(f"Produto econtrado: '{produto.nome}'")
        logger.debug(f"%d comentários encontrados" % len(comentarios))
        # retorna a representação de comentarios
        print(comentarios)
        return apresenta_comentarios(comentarios), 200
